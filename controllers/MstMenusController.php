    ...
    public function actionRoleSetting()
    {
        ...
        if($request->isPost){
            if(!empty($_POST['user_id'])) {
                /* $patient_info = $patient_model->checkPatientCode($_POST['patient_code']);
                 if (!empty($patient_info)) {
                     $this->redirect(['ip-bill-entry/billing', 'visit_id' => $patient_info[0]['id']]);
                 } else {
                     $info = Patient::find()->select(['name'])->where(['extid' => $_POST['patient_code']])->one();
                     if (!empty($info)) {
                         Yii::$app->session->setFlash('warning', 'This is  not an InPatient');
                     } else {
                         Yii::$app->session->setFlash('error', 'Invalid Patient Code');
                     }
               //  }*/
                $this->redirect(['mst-menus/edit-role', 'user_id' => $_POST['user_id']]);
            }else {
                Yii::$app->session->setFlash('warning','Enter the UserName');
            }
        }
        return $this->render('role_setting',['patient_info'=>$patient_info]);
    }
